...
abstract class BaseServicio extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Subservicio[] Collection to store aggregation of Subservicio objects.
	 */
	protected $collSubservicios;

	/**
	 * @var        Criteria The criteria used to select the current contents of collSubservicios.
	 */
	private $lastSubservicioCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collSubservicios = null;
			$this->lastSubservicioCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collSubservicios !== null) {
				foreach ($this->collSubservicios as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getSubservicios() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addSubservicio($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collSubservicios collection (array).
	 *
	 ...
	 * @see        addSubservicios()
	 */
	public function clearSubservicios()
	{
		$this->collSubservicios = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collSubservicios collection (array).
	 *
	 * By default this just sets the collSubservicios collection to an empty array (like clearcollSubservicios());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initSubservicios()
	{
		$this->collSubservicios = array();
	}

	/**
	 * Gets an array of Subservicio objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Servicio has previously been saved, it will retrieve
	 * related Subservicios from storage. If this Servicio is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array Subservicio[]
	 * @throws     PropelException
	 */
	public function getSubservicios($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ServicioPeer::DATABASE_NAME);
		}
		...
		if ($this->collSubservicios === null) {
			if ($this->isNew()) {
			   $this->collSubservicios = array();
				...
				$criteria->add(SubservicioPeer::SER_ID, $this->ser_id);

				SubservicioPeer::addSelectColumns($criteria);
				$this->collSubservicios = SubservicioPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				SubservicioPeer::addSelectColumns($criteria);
				if (!isset($this->lastSubservicioCriteria) || !$this->lastSubservicioCriteria->equals($criteria)) {
					$this->collSubservicios = SubservicioPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastSubservicioCriteria = $criteria;
		return $this->collSubservicios;
	}

	/**
	 * Returns the number of related Subservicio objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Subservicio objects.
	 * @throws     PropelException
	 */
	public function countSubservicios(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ServicioPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collSubservicios === null) {
			if ($this->isNew()) {
				...
				$count = SubservicioPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastSubservicioCriteria) || !$this->lastSubservicioCriteria->equals($criteria)) {
					...
					$count = count($this->collSubservicios);
				}
			} else {
				$count = count($this->collSubservicios);
			}
	...
	/**
	 * Method called to associate a Subservicio object to this object
	 * through the Subservicio foreign key attribute.
	 *
	 * @param      Subservicio $l Subservicio
	 * @return     void
	 * @throws     PropelException
	 */
	public function addSubservicio(Subservicio $l)
	{
		if ($this->collSubservicios === null) {
			$this->initSubservicios();
		}
		if (!in_array($l, $this->collSubservicios, true)) { // only add it if the **same** object is not already associated
			array_push($this->collSubservicios, $l);
			$l->setServicio($this);
		}
	}
	...
	{
		if ($deep) {
			if ($this->collSubservicios) {
				foreach ((array) $this->collSubservicios as $o) {
