...
abstract class BaseUsuario extends BaseObject  implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->usu_nombre = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->usu_apellido = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->usu_correo = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->usu_usuario = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->usu_password = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			...
			return $startcol + 7; // 7 = UsuarioPeer::NUM_COLUMNS - UsuarioPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setUsuId($pk);  //[IMV] update autoincrement primary key
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[1] => $this->getUsuNombre(),
			$keys[2] => $this->getUsuApellido(),
			$keys[3] => $this->getUsuCorreo(),
			$keys[4] => $this->getUsuUsuario(),
			$keys[5] => $this->getUsuPassword(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setUsuNombre($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUsuApellido($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setUsuCorreo($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setUsuUsuario($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setUsuPassword($arr[$keys[5]]);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setUsuApellido($this->usu_apellido);
